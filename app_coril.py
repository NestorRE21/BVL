...
RF,PPY = 0.02,252  # BVL da datos DIARIOS → 252 días/año
FICO_TK = "FICCMP13"
...
EJ = ["ALICORC1","ENGIEC1","BAP","CREDITC1","VOLCABC1","INRETC1"]
C_RV,C_RF,C_OPT = "#2E5E8C","#2CA02C","#D6604D"
BC = ["#888","#E377C2","#FF7F0E","#9467BD","#17BECF"]

for k,v in {"tickers":[],"rf_tickers":[],"include_fico":True,"benchmarks":["ALICORC1"],"views":[],"optimized":False,"result":None,
...
# ═══════════════════ BACKEND (BVL API) ═══════════════════════════════════════
def _get_creds():
    """Lee credenciales de st.secrets."""
    try:
...
def _period_to_dates(period):
    """Convierte '15y' a (start, end) en formato YYYYMMDD para la BVL."""
...
def dl_eq(tickers,period="15y"):
    """Descarga log-retornos de varios tickers desde la BVL."""
    cid,csec,key=_get_creds()
    if not cid: return None
    start,end=_period_to_dates(period)
    return bvl_data.download_prices(list(tickers),start,end,cid,csec,key)

@st.cache_data(show_spinner=False,ttl=600)
def dl_bk(tks,period="15y"):
    """Descarga benchmarks desde la BVL, devuelve dict {ticker: Serie log-ret}."""
    cid,csec,key=_get_creds()
    if not cid: return {}
    start,end=_period_to_dates(period)
    out={}
    lr=bvl_data.download_prices(list(tks),start,end,cid,csec,key)
    if lr is not None:
        for c in lr.columns:
...
def fetch_sec(tickers):
    """La BVL no da sectores; se marca como acción BVL."""
    return pd.Series({t:"Acción BVL" for t in tickers})
...
with tab1:
    ...
    with col_s:
        with st.form("add_ticker",clear_on_submit=True):
            ...
            q=fc1.text_input("Ticker BVL",placeholder="ALICORC1, ENGIEC1, NVDA, BAP…",
                             label_visibility="collapsed")
            submit=fc2.form_submit_button("➕ Añadir",use_container_width=True)
    ...
    st.caption("💡 Escribe el ticker exacto de la BVL (ej: ALICORC1, ENGIEC1, BAP, NVDA). "
               "La validez se confirma al descargar.")
